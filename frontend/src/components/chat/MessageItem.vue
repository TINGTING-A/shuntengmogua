      ...
      <div class="message-item__card">
        <template v-for="(turn, index) in turnsCache" :key="turn.id">
          ...
          <template v-if="turn.content">
            ...
            <div v-else class="message-item__text break-all whitespace-pre-wrap" @click="handleClick">
...
const renderSkillBadges = (text: string): string => {
  ...
  const escaped = text
    ...
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#x27;');
  // 再将转义后的 &lt;skill:xxx&gt; 替换为安全的徽标 HTML，并把裸网址转成可点击链接（点击走预览面板）
  return escaped
    .replace(
      /&lt;skill:([^&]+)&gt;/g,
    ...
    .replace(
      /(^|[^\w"'=])((?:https?:\/\/|www\.)[^\s<&，。；：！？、（）【】《》]+)/gi,
      (m: string, pre: string, url: string) => {
        const cleaned = url.replace(/[.,;:!?，。；：！？、）+）]+$/, '');
        const href = /^https?:\/\//i.test(cleaned) ? cleaned : 'https://' + cleaned;
        return `${pre}<a href="${href}" target="_blank" rel="noopener noreferrer">${cleaned}</a>`;
      }
    );
};
...
const handleClick = (event: MouseEvent) => {
  ...
  if (target.closest('.copy-code-button')) {
    ...
    if (codeElement) {
      ...
      // 同步 execCommand 优先（保留手势上下文），Clipboard API 兜底
      const doCopy = () => {
      ...
      if (window.isSecureContext && navigator.clipboard?.writeText) {
        navigator.clipboard.writeText(codeText)
          ...
          .catch(() => { doCopy() ? toast.success("代码已复制到剪贴板") : toast.error("复制失败"); });
      } else {
        copied = doCopy();
        copied ? toast.success("代码已复制到剪贴板") : toast.error("复制失败");
      }
    }
    return;
  }

  // 链接点击：拦截链接（http(s) 或上传目录文件），网页走右侧预览面板，文件直接下载
  ...
  if (linkEl) {
    ...
    const isUploadFile = href.startsWith('/uploads/');
    if (isHttp || isUploadFile) {
      event.preventDefault();
      event.stopPropagation();
      window.dispatchEvent(new CustomEvent('chat:open-url', { detail: { url: href } }));
    }
  }
};
