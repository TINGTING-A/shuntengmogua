<template>
  <div class="w-full flex flex-col items-center">
    <!-- 输入框区域 -->
    <div class="input-area p-[16px_12px_10px_12px] min-h-15 w-full" :class="styleClass">
      <!-- 文件列表显示区域 -->
      ...
      <div class="editor-container">
        ...
        <div v-if="skillPickerVisible" class="skill-picker">
          ...
          <div v-if="filteredSkills.length === 0" class="skill-picker-empty">
            {{ allSkills.length === 0 ? '暂无可用技能，按 Esc 关闭' : '未找到匹配的技能，按 Enter 直接发送' }}
          </div>
        </div>
      </div>
      ...
      <div class="input-actions w-full flex justify-between">
        <div class="tools left-tools flex gap-0.5 items-center">
          ...
          <!-- 语音输入按钮 -->
          <el-tooltip :content="recording ? '点击停止' : '语音输入'" placement="top">
            <el-button class="tool-btn" :class="{ 'recording-active': recording }" @click.stop="toggleRecording" text>
              <el-icon size="22">
                <Mic24Filled v-if="recording" style="color: #ef4444;" />
                <Mic24Regular v-else />
              </el-icon>
            </el-button>
          </el-tooltip>
          <!-- 翻译按钮（点击直接中英互译，自动检测方向） -->
          <el-tooltip content="翻译（自动中英互译）" placement="top">
            <el-button class="tool-btn" :loading="translating" @click.stop="handleTranslateCommand()" text>
              <span class="text-[13px] font-medium">中/英</span>
            </el-button>
          </el-tooltip>
        </div>
            ...
            <el-popover placement="top" :width="280" trigger="click">
              <template #reference>
                <el-button class="tool-btn" text>
                  <el-tooltip content="选择子Agent" placement="top">
                    <el-icon size="22"><component :is="PeopleTeam20Regular" /></el-icon>
                  </el-tooltip>
                  <span v-if="selectedAgentId" class="text-xs text-green-500 ml-1">{{ selectedAgentId.split('-')[0] }}</span>
                </el-button>
              </template>
              <div class="agent-selector-popover">
                <div class="text-xs text-gray-400 mb-2">选择子Agent执行任务</div>
                <div v-for="ag in availableAgents" :key="ag.id"
                  class="flex items-center gap-2 px-2 py-1.5 rounded cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700"
                  :class="{ 'bg-green-50 dark:bg-green-900/20': selectedAgentId === ag.id }"
                  @click="selectedAgentId = selectedAgentId === ag.id ? '' : ag.id">
                  <span class="text-sm">{{ ag.icon }}</span>
                  <div class="flex-1">
                    <div class="text-sm text-(--color-text)">{{ ag.name }}</div>
                    <div class="text-xs text-gray-400">{{ ag.category }}</div>
                  </div>
                </div>
                <div class="mt-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                  <el-button size="small" text @click="selectedAgentId = ''">清除选择</el-button>
                </div>
              </div>
            </el-popover>
          </div>
...
import {
  ...
  Send24Filled, Stop24Filled, Star24Regular, Star24Filled, Settings24Regular, BookSearch24Regular, FolderOpen24Regular, ChevronUpDown16Regular, PeopleTeam20Regular,
  Apps20Regular, Folder20Regular, Checkmark16Filled, Mic24Regular, Mic24Filled
} from '@vicons/fluent'
import {
  ThunderboltOutlined,
  SyncOutlined
...
const availableAgents = ref<Array<{id:string,name:string,icon:string,category:string}>>([])
...
// token 双存储兼容（未勾"记住我"时 token 在 sessionStorage）
...
async function startRecording() {
  try {
    ...
    ElMessage({ message: '录音中… 点击话筒停止', type: 'info' })
  } catch (e: any) {
    ElMessage.error('无法访问麦克风：' + (e?.message || '权限被拒绝'))
  }
}
...
async function transcribeAudio() {
  ...
  try {
    const res = await fetch('/api/v1/voice/stt', {
      ...
      body: JSON.stringify({ model: 'whisper-large-v3', audio: base64, language: 'zh' }),
    })
    if (res.ok) {
      ...
      ElMessage.error('语音识别失败（检查 Whisper 服务）')
    }
  } catch {
    ElMessage.error('语音识别服务不可用')
  }
}
...
// 翻译（中/英互译，LLM）—— 自动检测方向，点击直接转换
async function handleTranslateCommand() {
  const text = editor.value ? editor.value.getText() : ''
  if (!text.trim()) {
    ElMessage.warning('输入框为空，请先输入要翻译的内容')
    return
  }
  // 自动检测：中文字符多→译英，英文字符多→译中
  const chineseChars = (text.match(/[\u4e00-\u9fff]/g) || []).length
  const englishChars = (text.match(/[a-zA-Z]/g) || []).length
  const direction = chineseChars >= englishChars ? 'zh2en' : 'en2zh'
  ...
  try {
    ...
    if (data?.success && data.result) {
      if (editor.value) {
        const escaped = String(data.result)
          .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
        editor.value.commands.setContent(`<p>${escaped}</p>`, false)
      }
      ElMessage.success(direction === 'zh2en' ? '已翻译为英文' : '已翻译为中文')
    } else {
      ElMessage.error(data?.error || '翻译失败')
    }
  } catch {
    ElMessage.error('翻译服务不可用')
  } finally {
...
const loadAgents = async () => {
  try {
    ...
    const data = await res.json()
    availableAgents.value = (data.agents || []).map((a: any) => ({
      id: a.id, name: a.name, icon: Icons[a.category] || '🤖', category: a.category,
    }))
  } catch { /* */ }
};
