<template>
  ...
  <!-- 网页预览面板：消息中点击网址时在右侧以浏览器方式打开 -->
  <ChatBrowserPanel v-model:visible="browserPanelVisible" :url="browserPanelUrl" />
...
const ChatBrowserPanel = defineAsyncComponent(() => import("./ChatBrowserPanel.vue"));

// 网页预览面板状态
const browserPanelVisible = ref(false);
const browserPanelUrl = ref("");

// 监听消息中点击网址事件 → 打开右侧网页预览面板
const handleOpenUrl = (e: Event) => {
  const detail = (e as CustomEvent).detail;
  if (detail?.url) {
    // 文件下载链接（.pptx/.docx/.pdf 等或上传目录文件）：直接下载，不走预览面板
    if (isDownloadFileUrl(detail.url)) {
      window.open(detail.url, '_blank', 'noopener,noreferrer');
      return;
    }
    browserPanelUrl.value = detail.url;
    browserPanelVisible.value = true;
  }
};

// 判断是否为文件下载链接（办公文档、压缩包、媒体文件或上传目录资源）。
// PDF/图片走预览面板 iframe 原生渲染，不在此列
...
onMounted(async () => {
  // 注册消息中点击网址事件
  window.addEventListener('chat:open-url', handleOpenUrl);
...
// 组件卸载时取消监听
onUnmounted(() => {
  window.removeEventListener('chat:open-url', handleOpenUrl);
