<template>
  ...
  <div class="px-5 pb-2.5 w-full flex-1 flex flex-col items-center justify-center mb-20">
    <div class="flex flex-col items-center mb-8">
      <div class="sprite-banner mb-2">
        <SpriteCompanion :width="200" :height="200" :auto-rotate="true" />
      </div>
      <div v-if="spriteStore.message" class="sprite-mini-bubble mb-3">
        {{ spriteStore.message }}
      </div>
    </div>
    <h1 class="text-3xl mb-6 art-title">
      <span class="typewriter-text">{{ displayedText }}</span>
      <span class="typewriter-cursor" :class="{ 'cursor-blink': isTypingComplete }"></span>
    </h1>
    <!-- 已选角色显示（无框样式：头像 + 名称 + 可点击箭头） -->
    <div
      class="w-full max-w-200 -mb-6 flex items-center gap-3 p-2 pb-8 cursor-pointer transition-colors"
...
const currentModelId = computed(() => {
  ...
  if (currentCharacter.value?.modelId) {
    return currentCharacter.value.modelId;
  }
...
watch(() => currentSession.value.characterId, (newCharId, oldCharId) => {
  ...
  if (newCharId && newCharId !== oldCharId) {
    ...
    if (newCharacter) {
      ...
      // 第一优先级：角色的默认模型
      if (newCharacter.modelId) {
        selectedModelId = newCharacter.modelId;
      ...
      // 设置模型 ID
      if (selectedModelId) {
        currentSession.value.modelId = selectedModelId;
      }
  ...
  try {
    ...
    if (characters.value.length > 0) {
      ...
      if (targetCharacter) {
        ...
        // 第二优先级：角色的默认模型
        else if (targetCharacter.modelId) {
          selectedModelId = targetCharacter.modelId;
        }
        // 第三优先级：模型列表的第一个模型
        else if (models.value.length > 0) {
          selectedModelId = models.value[0].id;
...
const selectCharacter = (character: any): void => {
  ...
  if (character.modelId) {
    selectedModelId = character.modelId;
    // 清空用户手动选择的模型 ID（因为角色有推荐模型）
    userSelectedModelId.value = null;
  }
...
/* 站酷小薇艺术字体（官方完整分块字库，覆盖全部汉字） */

/* 艺术标题 */
.art-title {
  font-family: 'Ma Shan Zheng', 'ZCOOL XiaoWei', 'KaiTi', 'STKaiti', serif;
