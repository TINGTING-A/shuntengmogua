<template>
  <div v-if="visible" class="setup-guide-container" :style="{ top: guidePosition.y + 'px', left: guidePosition.x + 'px' }" @mousedown="startDrag">
    <div class="guide-header" @mousedown="startDrag">
      <span class="guide-title">✨ 欢迎使用顺藤摸瓜</span>
...
onMounted(() => {
  // 2026-08-07 修复：不再自动弹出引导弹窗（避免遮挡新建任务页中间的蛋精灵和智能助手卡片）
  // 引导仅通过右下角 FAB 手动打开
...
// 监听认证状态：登录成功后不再自动弹出引导弹窗（2026-08-07 修复遮挡）
watch(() => authStore.isAuthenticated, () => {
  // 保留：登录后同步步骤状态，但不强制显示弹窗
})
