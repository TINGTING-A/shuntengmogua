  ...
  <div class="h-full w-full flex flex-col min-h-0">
    ...
    <div class="flex flex-col flex-1 p-3 md:max-w-260 md:mx-auto w-full">
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <el-button link type="primary" class="docs-btn text-sm" @click="openDocs">
          <template #icon>
            <el-icon :size="16">
      ...
      <div class="flex-1 overflow-y-auto pt-4">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
          ...
          <div v-if="!loading && characters.length === 0"
            ...
            <p class="text-lg">暂无智能助手</p>
            <p class="text-sm mt-1">点击上方按钮创建第一个智能助手</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <el-dialog v-model="docsVisible" width="640px" destroy-on-close>
      <template #header>
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <div class="text-xs text-gray-400">智能助手使用说明</div>
          </div>
        </div>
      </template>
      ...
      <div class="docs-body text-sm space-y-4 max-h-70vh overflow-y-auto pr-2">
        <div>
          ...
          <p class="text-gray-500 dark:text-gray-400">智能助手是 AI 对话的「人格角色」。每个助手有独立的头像、名称、描述和<b>系统提示词</b>，决定 AI 的身份、语气与能力边界。聊天时先选择角色，AI 就以该角色身份与你对话。</p>
        </div>
        ...
        <div>
          ...
          <ol class="list-decimal list-inside space-y-1 text-gray-500 dark:text-gray-400">
            <li>点击「新建智能助手」，设置头像、名称、描述与分组</li>
            <li>编写「系统提示词」：定义角色身份、说话风格、知识范围（越具体效果越好）</li>
            <li>回到聊天页，点击顶部角色选择器，选中刚创建的助手开始对话</li>
            <li>可随时编辑助手或按分组管理多个角色</li>
          </ol>
        </div>

        <div>
          <h4 class="font-semibold mb-1 text-(--color-text)">💡 提示词技巧</h4>
          <ul class="list-disc list-inside space-y-1 text-gray-500 dark:text-gray-400">
            <li>身份：你是资深中医专家 / 你是一名产品经理……</li>
            <li>语气：专业严谨 / 轻松幽默 / 温柔耐心……</li>
            <li>规则：只回答中医相关问题、始终用中文、先给结论再展开……</li>
            <li>系统提示词越清晰，AI 表现越稳定</li>
          </ul>
        </div>

        <div>
          <h4 class="font-semibold mb-1 text-(--color-text)">📁 分组管理</h4>
          <ul class="list-disc list-inside space-y-1 text-gray-500 dark:text-gray-400">
            <li>顶部标签按分组筛选助手，支持「新建分组」归类</li>
            <li>右键分组可重命名 / 删除，删除分组后助手变为未分组</li>
            <li>默认分组「全部」显示所有助手</li>
          </ul>
        </div>
      </div>
...
const docsVisible = ref(false)  // 使用说明弹窗
...
const loadCharacters = async (groupId?: string): Promise<void> => {
  ...
  try {
    ...
    console.error('获取智能助手列表失败:', error)
    toast.error('获取智能助手列表失败')
  } finally {
    loading.value = false
  }
}
...
const handleDeleteGroup = async (group: CharacterGroup): Promise<void> => {
  try {
    const result = await confirm('确认删除', `确定要删除分组「${group.name}」吗？该分组下的智能助手将变为未分组状态。`)
    if (result) {
...
const deleteCharacter = async (character: any): Promise<void> => {
  try {
    const result = await confirm('确认删除', `确定要删除智能助手「${character.title}」吗？此操作不可撤销。`)
    if (!result) {
      return
    }
    await apiService.deleteCharacter(character.id)
    await loadCharacters(currentGroupId.value || undefined)
    toast.success('智能助手删除成功')
  } catch (error: any) {
    toast.error('删除失败')
    console.error('删除智能助手失败:', error)
  }
}
