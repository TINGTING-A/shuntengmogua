  ...
  <el-dialog v-model="docsVisible" width="640px" destroy-on-close>
    <template #header>
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <div class="text-xs text-gray-400">知识库使用说明</div>
        </div>
      </div>
    </template>
    ...
    <div class="docs-body text-sm space-y-4 max-h-70vh overflow-y-auto pr-2">
      <div>
        ...
        <p class="text-gray-500 dark:text-gray-400">知识库让 AI 基于<b>你自己的文档</b>回答问题（RAG 检索增强）。上传资料后，AI 在回答前会先检索相关知识，答案更准确、更有依据。</p>
      </div>
      ...
      <div>
        ...
        <ol class="list-decimal list-inside space-y-1 text-gray-500 dark:text-gray-400">
          <li>点击「新建知识库」，填写名称与描述，可调整分块大小与重叠</li>
          <li>创建后点击知识库卡片进入，上传文档（自动分块 + 向量化）</li>
          <li>在「文件」页管理文档树，在「搜索」页按关键词检索内容</li>
          <li>聊天时点击输入框的「知识库」按钮引用，或在创建机器人时指定</li>
        </ol>
      </div>

      <div>
        <h4 class="font-semibold mb-1 text-(--color-text)">📄 支持能力</h4>
        <ul class="list-disc list-inside space-y-1 text-gray-500 dark:text-gray-400">
          <li>文本/文档上传，自动分块并向量化存储</li>
          <li>文件树懒加载管理，支持文件夹结构与文件分块查看</li>
          <li>关键词搜索 + 语义检索（BGE-M3 向量）</li>
          <li>分块大小 / 重叠大小可配置，按文档类型调优</li>
        </ul>
      </div>

      <div>
        <h4 class="font-semibold mb-1 text-(--color-text)">💡 引用方式</h4>
        <ul class="list-disc list-inside space-y-1 text-gray-500 dark:text-gray-400">
          <li>聊天：输入框左侧「知识库」按钮，选择要引用的知识库（可多选）</li>
          <li>机器人：创建/编辑机器人时勾选知识库，群聊中自动基于文档回答</li>
          <li>选择记录会保存，下次对话自动恢复</li>
        </ul>
      </div>
    </div>
