<template>
    <div class="h-full overflow-hidden flex flex-col">
        <PageHeader title="能力中心" />
        <div class="h-full px-4 flex flex-col flex-1 overflow-auto">
            <div class="flex-1 flex flex-col">
                <div class="sticky top-0 z-10 bg-(--color-bg)">
                    <el-tabs v-model="currentTabValue" @tab-change="handleTabChange" class="capability-tabs">
                ...
                <div class="flex-1 py-3">
                    <template v-if="currentTabValue === 'connectors'">
                        <ConnectorMarketPage />
                    </template>
...
import ConnectorMarketPage from '@/components/connector-market/ConnectorMarketPage.vue'
import MCPServers from '@/components/plugins/MCPServers.vue'
import LocalTools from '@/components/plugins/LocalTools.vue'
import Skills from '@/components/plugins/Skills.vue'
...
const getDefaultTabPath = () => {
    return sidebarItems[0]?.path || 'connectors'
}
...
const handleTabChange = (tabName: string | number) => {
    ...
    router.replace({ name: 'CapabilityCenter', params: { tab: tabPath } })
}
...
onMounted(() => {
    if (!route.params.tab) {
        ...
        router.replace({ name: 'CapabilityCenter', params: { tab: defaultTab } })
    } else {
