...
function createMarkedInstance(options?: MarkdownOptions): Marked {
  ...
  <path fill="currentColor" d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"/>
    ...
    // 自定义裸 URL 扩展：修复 marked 内置 autolink 会把 URL 后面的中文/标点吞进 href 的问题
    // （如 "https://www.image-net.org/，语音交互内容" 会被整体编成 href），
    // 遇到中文/空白/常见标点即截断，保证 href 干净可预览；同时支持 www. 开头无协议的网址
    const stmgAutolink = {
      name: 'stmgAutolink',
      level: 'inline',
      start(src: string) {
        const idx = src.search(/https?:\/\//i);
        const idx2 = src.search(/www\./i);
        if (idx < 0) return idx2 >= 0 ? idx2 : undefined;
      ...
      tokenizer(src: string) {
        ...
        return { type: 'stmgAutolink', raw, href, text: cleaned };
      },
      renderer(token: any) {
        return `<a href="${token.href}" target="_blank" rel="noopener noreferrer">${token.text}</a>`;
      },
    };
    marked.use({ extensions: [stmgAutolink] })
