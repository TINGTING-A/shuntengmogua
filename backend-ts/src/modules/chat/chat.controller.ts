...
export class ChatController {
  ...
  async streamMessageWithMaf(
    ...
    const { sessionId, userMessage, thinkingEffort, resumeData } = body;

    this.logger.log(
      `[MAF Stream] userId=${userId} sessionId=${sessionId}`,
    );
    ...
    try {
      await this.chatRunner.startStreamWithMaf(
        {
          sessionId,
          userId,
          userMessage,
          ...
          resumeData,
          source: { clientId: req.headers["x-client-id"] as string },
        },
        {
          onEvent: (data: string) => {
            res.write(`data: ${data}\n\n`);
