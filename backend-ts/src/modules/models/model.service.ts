...
export class ModelService {
  ...
  async testProviderConnection(data: any) {
    ...
    // 关键：去除 key 首尾空格/换行（用户复制粘贴常带，导致 401 误判）
    const key = (apiKey || "").trim();
    const baseUrl = (apiUrl || "").trim().replace(/\/+$/, "");

    // 诊断日志：记录 key 格式（脱敏，帮助排查 401 问题）
    this.logger.log(
      `测试连接: provider=${provider || "?"} url=${baseUrl} key格式=${key.slice(0, 6)}...(${key.length}字符, 前缀=${key.startsWith("sk-") ? "sk-" : key.slice(0, 3)})`,
    );
    ...
    try {
      // 尝试创建一个临时的 OpenAI 客户端进行测试（带超时，避免网络不通时挂起）
      const client = new OpenAI({
        apiKey: key,
        baseURL: baseUrl,
        timeout: 15000, // 15 秒超时（OpenAI 官方域名国内不通时会快速失败）
        maxRetries: 0, // 不重试，快速返回结果
      });
      ...
      if (status === 401 || msg.includes("401")) {
        return {
          success: false,
          message:
            "API Key 无效（401）：请检查密钥是否正确完整，注意复制时不要带空格或换行",
        };
      }
      if (status === 403) {
        return { success: false, message: "API Key 无权限（403）：请检查账户权限或余额" };
      }
      if (status === 404) {
        return { success: false, message: "API 地址错误（404）：请检查 apiUrl 是否包含正确的 /v1 路径" };
      }
      if (
        ...
        return {
          success: false,
          message: "连接超时：网络无法访问该 API 地址（OpenAI 官方域名在国内可能无法直连，可改用中转地址）",
        };
      }
      if (msg.includes("ENOTFOUND") || msg.includes("getaddrinfo")) {
        return { success: false, message: "无法解析域名：请检查 API 地址是否正确" };
      }
      if (msg.includes("ECONNREFUSED")) {
        return { success: false, message: "连接被拒绝：请检查 API 地址和端口" };
      }
      return { success: false, message: `连接失败: ${msg.slice(0, 200)}` };
    }
  }
  ...
  /**
   * 从远程 API 获取可用模型列表
   * 远程拉取失败时降级返回供应商内置模板模型（市面主流模型），保证搜索模型始终可用
   */
  async getRemoteModels(providerId: string) {
    ...
    // 将模板模型转换为统一格式
    const templateModels = supplierModels.map((m: any) => ({
      modelName: m.modelName,
      modelType: m.modeType || m.modelType || "text",
      config: m.config || {
    ...
    try {
      ...
      const remoteModels = response.data.map((model: any) => {
        // 尝试从供应商模型中匹配配置
      ...
      // 合并远程模型与模板模型（远程优先，模板补充市面上未返回的模型）
      const mergedNames = new Set(remoteModels.map((m: any) => m.modelName));
      const mergedModels = [
        ...remoteModels,
        ...templateModels.filter((m: any) => !mergedNames.has(m.modelName)),
      ];

      // 返回分页格式，与其他列表接口保持一致
      return createPaginatedResponse(mergedModels, mergedModels.length);
    } catch (error) {
      // 远程拉取失败：降级返回内置模板模型，保证用户仍可搜索添加市面主流模型
      this.logger.warn(
        `Failed to fetch remote models for provider ${providerId}, falling back to template models:`,
        error,
      );
      return createPaginatedResponse(templateModels, templateModels.length);
    }
  }
