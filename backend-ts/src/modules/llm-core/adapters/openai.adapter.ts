...
export class OpenAIAdapter implements IProtocolAdapter {
  ...
  private handleError(error: any, isStream: boolean) {
    ...
    if (error.message?.includes("fetch") || error.message?.includes("Connect") || error.message?.includes("ENOTFOUND"))
      throw new Error("无法连接LLM服务, 请检查API地址和网络");
    if (error.cause?.code === "ECONNREFUSED" || error.cause?.code === "ENOTFOUND")
      throw new Error("LLM服务连接失败, 请检查API地址配置");
    throw error;
  }
